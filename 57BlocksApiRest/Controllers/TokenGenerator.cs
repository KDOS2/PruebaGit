namespace _57BlocksApiRest.Controllers
{
    ...
    {
        public static string GenerateTokenJwt(string username, Configuration_ config)
        {
            var secretKey = config.JWT_SECRET_KEY;
            var audienceToken = config.JWT_AUDIENCE_TOKEN;
            var issuerToken = config.JWT_ISSUER_TOKEN;
            var expireTime = config.JWT_EXPIRE_MINUTES;
